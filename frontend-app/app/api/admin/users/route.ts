...
// 3 query rời rồi merge bằng Map thay vì JOIN — join trực tiếp tasks + reminders sẽ fan-out
// (nhân chéo số dòng), COUNT DISTINCT giải quyết được nhưng phức tạp hơn cần thiết.
// taskCounts/reminderCounts tính trên toàn bộ user (không phân trang) vì chỉ dùng để tra cứu,
// rẻ hơn nhiều so với lấy hết cột dữ liệu.
export const GET = withAdminContext(async (req, { db }) => {
  ...
  // Không lọc isNull(deletedAt) — admin cần thấy cả tài khoản đã xoá mềm để khôi phục lại được.
  ...
  // Không lọc deletedAt — reminders không có soft-delete (luôn hard delete), khác tasks.
