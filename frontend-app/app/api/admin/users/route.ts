...
// 3 separate queries merged with a Map instead of a JOIN since a direct join would fan out, taskCounts/reminderCounts are computed across all users.
export const GET = withAdminContext(async (req, { db }) => {
  ...
  // Doesn't filter isNull(deletedAt) since admins need to see soft-deleted accounts too in order to restore them.
  ...
  // Doesn't filter deletedAt since reminders have no soft-delete, always a hard delete, unlike tasks.
