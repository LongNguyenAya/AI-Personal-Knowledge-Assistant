...
// The badge pill takes its color/icon from the real numbers (trend.slope or moving average), not guessed from the trendMessage string.
...
// Props can come from old toolResults from when createChart's output shape was still missing xAxisType/outliers, needs safe defaults.
...
function BarOrLineChart({ chartType, xAxisType, data, trend, trendMessage, outliers, movingAverage, softForecast }: ChartBlockProps) {
  ...
  // Only a time series has future forecast points, futureLower/futureUpper default to [] for old toolResults.
  ...
  // softForecast only exists when trend=null so it never shows alongside futurePoints, but it's still added to futureCount for xFor().
  ...
  // "category": splits evenly into bands with each bar centered in its band, unlike "time" which spreads evenly to both edges to leave room for the forecast.
  ...
  // futureUpper has to be included in allValues, otherwise the confidence band could get clipped when its upper bound exceeds the max value.
  ...
  // The confidence band widens gradually from the last real point (width=0) out to the upper/lower bounds, following predictionMargin.
  const bandPath =
  ...
  // Only has a value when OLS doesn't reach statistical significance, draws an extra smoothed line in a different color to still show the recent trend.
  const maPath =
    isLine && movingAverage
      ? data.map((_, i) => `${i === 0 ? "M" : "L"} ${xFor(i).toFixed(1)},${yFor(movingAverage[i]).toFixed(1)}`).join(" ")
      : null;

  // The "soft" Holt-linear forecast only appears alongside maPath, drawn as an orange dashed line to avoid confusion with OLS (blue dashed).
  ...
  // Thins out x-axis labels when there are too many points, always keeps the first/last, category (usually few groups) always shows every one.
...
function PieChart({ data }: { data: ChartDatum[] }) {
  ...
  // When 1 group is 100% the arc angle is 360°, a degenerate arc path renders nothing, so a circle is drawn instead of a path.
