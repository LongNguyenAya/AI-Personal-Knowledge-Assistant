...
async function main() {
  ...
  for (let i = 0; i < CASES.length; i++) {
    ...
    if (!reminderCall) {
      ...
      rows.push(`| ${c.message} | ${c.note} | ${expectedUtc.toISOString()} | (không gọi tool) | SAI |`);
      continue;
    }
    ...
    rows.push(`| ${c.message} | ${c.note} | ${expectedUtc.toISOString()} | ${actualUtc.toISOString()} | ${pass ? "OK" : "SAI"} |`);
  ...
  writeEvalReport("action-consistency", {
    title: "createReminder time-zone consistency",
    summary: `Độ chính xác quy đổi giờ: ${correct}/${CASES.length}. Đáp án đúng tính bằng code (cộng/trừ UTC+7), không qua AI, kể cả case biên giờ rất sớm sáng mai (dễ tính nhầm ngày UTC).`,
    table: { headers: ["Câu nói", "Ghi chú", "Mong đợi (UTC)", "Thực tế (UTC)", "Kết quả"], rows },
    notes: failures,
  });
